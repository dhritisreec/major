...
def run_experiment(args, group=None, gpu=None):
    args = copy.deepcopy(args)

    if args['model'] != 'sgc':
        args['K'] = 0

    log_dir = os.path.join('results', 'malnet_tiny={}'.format(args['malnet_tiny']),
                           'group={}'.format(args['group']),
                           'train_ratio={}'.format(args['train_ratio']),
                           'node_feature={}'.format(args['node_feature']),
                           'directed_graph={}'.format(args['directed_graph']),
                           'remove_isolates={}'.format(args['remove_isolates']),
                           'lcc_only={}'.format(args['lcc_only']),
                           'add_self_loops={}'.format(args['add_self_loops']),
                           'model={}'.format(args['model']),
                           'K={}'.format(args['K']),
                           'hidden_dim={}'.format(args['hidden_dim']),
                           'num_layers={}'.format(args['num_layers']),
                           'lr={}'.format(args['lr']),
                           'dropout={}'.format(args['dropout']),
                           'epochs={}'.format(args['epochs']))

    data_dir = os.path.join('data', 'malnet_tiny={}'.format(args['malnet_tiny']),
                            'group={}'.format(args['group']),
                            'train_ratio={}'.format(args['train_ratio']),
                            'node_feature={}'.format(args['node_feature']),
                            'directed_graph={}'.format(args['directed_graph']),
...
def main():
    ...
    gpus = [0]
    groups = ['type']
    args = {'log_dir': 'results/malnet_tiny=False/group=family/train_ratio=1.0/node_feature=ldp/directed_graph=True/remove_isolates=False/lcc_only=False/add_self_loops=True/model=gcn/K=0/hidden_dim=64/num_layers=3/lr=0.001/dropout=0.5/epochs=5'}
